...
namespace gl {
	...
	void HelloTransform::Init()
	{
		...
		std::string path = "../";
		 // ajouter mesh_ = std::make_unique<Mesh>(path + "data/meshes/Cube.obj");
		...
		//shaders_->SetInt("object_material.diffuse", 0);
		shaders_->SetInt("Diffuse", 0);
		texture_specular_->Bind(1);
		//shaders_->SetInt("object_material.specular", 1);
		shaders_->SetInt("Specular", 1);
		//shaders_->SetInt("object_material.shiningness", 2);
	...
	void HelloTransform::Update(seconds dt)
	{
		...
		glBindVertexArray(VAO_);
		IsError(__FILE__, __LINE__);
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0); // nbr triangle * 3 = 6 -> for mesh = 
		IsError(__FILE__, __LINE__);
	}
